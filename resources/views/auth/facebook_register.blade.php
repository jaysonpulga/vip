...
<body class="hold-transition register-page body_register">
   <form method="POST" action="{{ route('facebook_register') }}" id='form_register'>
      @csrf


    <input type="hidden" name="user_id" id="user_id"  value="{{@$data->id}}" />
    <input type="hidden" name="providerUserid" id="providerUserid"  value="{{@$providerUserid}}" />
     ...
     <fieldset style='' id='accoun_settings'>
	  ...
	  <div style='text-align:left'>
       ...
       <div class="form-group has-feedback">
		...
		<div class='col-lg-6'>  
		<input  placeholder="First name"  id="name" type="text"     class="required form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ @$data->name }}" {{ !empty(@$data->name) ? "readonly" : "" }}   required autofocus>
         ...
         <div class='col-lg-6'>  
		 <input  placeholder="Last name"  id="lastname" type="text" class="required form-control{{ $errors->has('v') ? ' is-invalid' : '' }}" name="lastname" value="{{  @$data->lastname }}" {{ !empty(@$data->lastname) ? "readonly" : "" }}   required autofocus>
      ...
      <div class="form-group has-feedback">
         <input placeholder="Email" id="email" type="email" class="required form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{  @$data->email }}" {{ !empty(@$data->email) ? "" : "" }}>
     ...
     var formData = new FormData($('#form_register')[0]);
             ...
             event.preventDefault();
               ...
               var response = jQuery.ajax({
                  url: "{{ route('validateusers_update') }}",
                  method: 'post',
                  data: formData,
                  contentType: false,
                 cache: false,
