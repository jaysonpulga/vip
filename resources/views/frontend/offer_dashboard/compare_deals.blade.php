...
<div class='featured_body compare_deals hide'>
	<div class="row">
	    
	    <div style="margin-left:15px;color:rgb(99, 107, 111);margin-bottom:10px">
	        <span style="font-size:18px">Compare Product</span><br>
             <small> Limited invitations</small> 
	    </div>
	    
		<div id='offerData4'></div>
		...
		success: function(details)
		{
		  //  var compare_count = 0;
			if(details.length > 0 ) 
			{
				...
				// 	    compare_count++;
				// 	}
					 htmlData += currentOfferTemplate2(value.campaign_data,value.image_path,value.schedule_offer,value.product_price_data,value.offer_data);
...
// 			}
			...
			$("#offerData4").empty().hide().append(htmlData).animate({ opacity: "show" }, "slow");
...
<script>
function currentOfferTemplate2(value,image,schedule_offer,product_price_data,offer_data)
{
	var image_path = '';
	var imageDiv = '';
	var Status_msg ="";
	...
	var template = '';
	if(value.campaign_type == 'compare campaign'){
    	 template = 
       `<div id="${value.id}">
    		...
    		<div class="col-md-3">
    			...
    			<a href="{{asset('campaign/getdata/compare/productdetails/${value.id}')}}" style="width:100%" class="btn btn-default btn-flat">View Details</a>	
    			</div>
    		</div>
       </div>`;
       $('.featured_body.compare_deals').removeClass('hide');
	}
	return template;
